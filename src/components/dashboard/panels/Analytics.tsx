...
const RoomDetailModal: React.FC<RoomDetailModalProps> = ({ room, onClose, token }) => {
    ...
    const fetchRecordings = async () => {
        try {
            ...
            const roomInfo = await flatService.getRoomInfo(room.roomUUID, token);

            if (roomInfo.roomInfo.hasRecord && roomInfo.roomInfo.records) {
                setRecordings(roomInfo.roomInfo.records);
                    ...
                    <div className="border-b px-6 pt-4">
                        <div className="flex space-x-1">
                            ...
                            <button
                                ...
                                className={`px-4 py-2 font-medium ${activeTab === 'recordings'
                                    ? 'text-blue-600 border-b-2 border-blue-600'
                                    : 'text-gray-600 hover:text-gray-800'
                                }`}
                            >
